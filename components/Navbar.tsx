...
import { Menu, X, ChevronDown, Compass, MapPin, Award, Sliders } from "lucide-react";
...
const Navbar = () => {
  ...
  const [isProgramsMobileOpen, setIsProgramsMobileOpen] = useState(false); // Mobile sub-accordion
  ...
  useEffect(() => {
    const fetchTaxonomies = async () => {
      try {
        const [typeRes, skillRes, regionRes] = await Promise.all([
        ...
        if (typeRes.ok && skillRes.ok && regionRes.ok) {
    ...
    <header className="sticky top-0 z-50 w-full bg-secondary/90 backdrop-blur-md border-b border-primary/10 select-none">
      <nav className="max-w-7xl mx-auto h-20 px-6 md:px-12 flex justify-between items-center relative">
        ...
        <ul className="hidden lg:flex items-center space-x-8 font-serif text-xs tracking-widest uppercase h-full">
          ...
          {/* Direct Link: Expeditions (ID: 191) */}
          <li className="h-full flex items-center">
            <Link
              href="/programs?program_type=expedition"
              className="hover:text-accent transition-colors py-2 border-b-2 border-transparent text-primary/90"
            >
              Expeditions
            </Link>
          </li>

          {/* Direct Link: Cultural Heritage (ID: 194) */}
          <li className="h-full flex items-center">
            <Link
              href="/programs?program_type=cultural-immersion"
              className="hover:text-accent transition-colors py-2 border-b-2 border-transparent text-primary/90"
            >
              Cultural Heritage
            </Link>
        ...
        >
          ...
          <div className="p-6 space-y-6">
            <ul className="space-y-5 font-serif text-sm tracking-widest uppercase">
              ...
              <li>
                <Link href="/programs?program_type=expedition" className="block py-1 text-primary/90">
                  Expeditions
                </Link>
              </li>

              <li>
                <Link href="/programs?program_type=cultural-immersion" className="block py-1 text-primary/90">
                  Cultural Heritage
                </Link>
              </li>
