...
import { Search, X, SlidersHorizontal, Compass, Shield, Award } from "lucide-react";
...
const EquipmentContent = () => {
  ...
  // Load live shop database on mount
  useEffect(() => {
  ...
  return (
    <Suspense fallback={
      <div className="w-full min-h-screen bg-secondary flex items-center justify-center">
        <span className="font-serif text-sm tracking-widest uppercase text-primary/50 animate-pulse">Loading Catalog...</span>
      </div>
    }>
      <EquipmentContent />
    </Suspense>
