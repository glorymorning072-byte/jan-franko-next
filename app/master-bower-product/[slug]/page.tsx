...
import { ChevronLeft, Compass, Shield, Award, CheckCircle2, Sliders, Calendar, ArrowRight } from "lucide-react";
...
const BowyerProductContent = () => {
  ...
  const handleSelectionChange = (fieldId: string, value: string | number) => {
    setSelections((prev) => ({
      ...prev,
      [fieldId]: value
    }));
  ...
  return (
    <div className="w-full min-h-screen bg-secondary text-primary select-text relative pb-24">
      ...
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 mt-6">
        ...
        {/* Left Side (45%): Gallery, Thumbnails, & Specifications */}
        ...
        {/* Right Side (55%): Title, Description, Custom Build Configurator */}
        <div className="lg:col-span-7 space-y-8 detail-fade-in">
          ...
          <div className="bg-white border border-primary/5 p-8 rounded-3xl shadow-sm space-y-3">
            <span className="text-[10px] uppercase tracking-widest text-[#5c4629] font-serif font-bold">
              {product.acf?.product_subtitle || "Custom Post Type Creation"}
            </span>
          ...
          {/* Interactive Custom bow Configurator Panel */}
          {product.acf?.configurator_fields && product.acf.configurator_fields.length > 0 && (
            <div className="bg-white border border-primary/5 p-8 rounded-3xl shadow-sm space-y-6">
              ...
              <div className="space-y-6">
                {product.acf.configurator_fields.map((field) => (
                  <div key={field.field_id} className="space-y-2.5">
                    <div className="flex justify-between items-center">
                      <label className="text-xs font-semibold text-primary/70 font-sans">
                        {field.field_label}
                      </label>
                      <span className="text-[11px] font-bold text-accent font-serif bg-accent/10 px-2.5 py-0.5 rounded-full">
                        {selections[field.field_id]}
                      </span>
                    </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {field.field_options.map((opt) => (
                          <button
                            ...
                            className={`px-4 py-2 border rounded-xl text-left text-xs font-sans transition-all ${
                              selections[field.field_id] === opt.option_label
                                ? "border-accent bg-accent/5 ring-1 ring-accent text-primary font-semibold"
                                : "border-primary/5 hover:border-accent/30 text-primary/70 bg-secondary/50"
                            }`}
                          >
                            {opt.option_label}
                          </button>
              ...
              <div className="bg-[#0e3b2e] p-6 rounded-2xl text-white flex flex-col sm:flex-row items-center justify-between gap-6 shadow-inner mt-8">
                <div className="space-y-1 text-center sm:text-left">
                  ...
                  <p className="text-xs text-white/70 font-sans leading-relaxed">
                    Submit your configured specs to our bowyers for direct queue slot availability and custom pricing.
                  </p>
                </div>
                <button
      ...
      {/* 3. Custom Lineage & Craft Reliability Sections */}
      ...
      {/* 4. Consultation Inquiry Section */}
